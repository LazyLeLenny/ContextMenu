...
class FifthQuizScreen : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        enableEdgeToEdge()
        setContentView(R.layout.activity_fifth_quiz_screen)
    ...
    private val radioButtonClickListener =
        View.OnClickListener { view ->
            ...
            when(radioButton.id) {
                R.id.threeRB -> {
                    result = intent.getIntExtra("resultFour", 0) + 100
                    Toast.makeText(this, "Верно! $result", Toast.LENGTH_LONG).show()
                    startActivity(
                        Intent(this, ResultActivity::class.java)
                            .putExtra("resultFinal", result))
                } else -> {
                result = intent.getIntExtra("resultFour", 0)
