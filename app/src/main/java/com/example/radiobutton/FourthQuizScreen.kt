...
class FourthQuizScreen : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        enableEdgeToEdge()
        setContentView(R.layout.activity_fourth_quiz_screen)
    ...
    private val radioButtonClickListener =
        View.OnClickListener { view ->
            ...
            when(radioButton.id) {
                R.id.threeRB -> {
                    result = intent.getIntExtra("resultThree", 0) + 100
                    Toast.makeText(this, "Верно! $result", Toast.LENGTH_LONG).show()
                    startActivity(
                        Intent(this, FifthQuizScreen::class.java)
                            .putExtra("resultFour", result))
                } else -> {
                result = intent.getIntExtra("resultThree", 0)
